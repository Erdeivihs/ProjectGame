...
export { plataformeogame };

async function borrar(dato) {
  await getEliminar("plataformeo", dato, localStorage.getItem("access_token"));
}

async function plataformeogame() {
  let access_token = localStorage.getItem("access_token");
  let games = await getData('plataformeo?select=*', access_token);
